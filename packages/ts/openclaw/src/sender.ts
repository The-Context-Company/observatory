...
/**
 * Resolves the TCC API key from config or environment.
 */
...
/**
 * Resolves the TCC endpoint URL with automatic dev/prod routing.
 */
...
export function createSender(
  ...
  if (!apiKey) {
    console.warn(
      "[TCC OpenClaw] No API key found. Set TCC_API_KEY or pass apiKey in config. Telemetry will not be sent."
    );
    return async () => {};
  }
  ...
  return async (payload: BatchPayload): Promise<void> => {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        if (!isRetryable(res.status)) {
          console.error(
            `[TCC OpenClaw] Ingestion failed (${res.status}): ${text}`
          );
          return;
        }
    ...
    console.error(
      `[TCC OpenClaw] Ingestion failed after ${MAX_RETRIES + 1} attempts:`,
      lastError
    );
  };
}
