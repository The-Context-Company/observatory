...
export async function setup(ctx: WizardContext): Promise<void> {
  ...
  const usageGlobal = `import { TCCCallbackHandler, setGlobalHandler } from "@contextcompany/langchain";

// Option A: Set globally (all LangChain calls are tracked)
setGlobalHandler(new TCCCallbackHandler());`;

  const usagePerCall = `// Option B: Pass per invocation
const handler = new TCCCallbackHandler();
const result = await model.invoke("Hello!", { callbacks: [handler] });`;

  p.note(
    [
      `${pc.bold("1.")} Add the TCC callback handler to your LangChain code:`,
      "",
      pc.dim(usageGlobal),
      "",
      pc.dim(usagePerCall),
