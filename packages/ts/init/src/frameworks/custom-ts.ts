...
export async function setup(ctx: WizardContext): Promise<void> {
  ...
  const usageCode = `import { configure, run } from "@contextcompany/custom";

// Initialize
configure({ debug: true });

// Create a run and record events
const r = run({ sessionId: crypto.randomUUID() });
r.prompt("User message here");

// Record steps, tool calls, etc.
const s = r.step();
s.prompt("...");
s.response("...");
s.end();

r.response("Agent response");
await r.end();`;

  p.note(
    [
      `${pc.bold("1.")} Use the builder pattern to instrument your agent:`,
      "",
      pc.dim(usageCode),
      "",
      ...(ctx.apiKey
